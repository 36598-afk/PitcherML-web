    ...
    <>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4">
        ...
        <h2 className="text-2xl font-bold text-center mb-2">
          {showEmail ? (isLogin ? <span>{auth.headingLogin}</span> : <span>{auth.headingSignup}</span>) : <span>{auth.headingOAuthOnly}</span>}
        </h2>
        {!showEmail && <p className="text-gray-600 text-center mb-6"><span>{auth.subheadingOAuthOnly}</span></p>}
          ...
          <>
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              ...
              <div>
                ...
                {!isLogin && (
                  <p className="mt-1 text-xs text-gray-500"><span>{auth.passwordHint}</span></p>
                )}
              </div>
